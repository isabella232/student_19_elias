...
testReconnect(){
    for s in 1 2; do
        ...
        runSrv $s
        testOK runCl 1 sign foo.txt
        pkill -9 -f ./blscosi_hybrid_rumor
    done
}
...
testServerCfg(){
    runSrvCfg 1
    pkill -9 blscosi_hybrid_rumor
    testFile srv1/private.toml
}

testBuild(){
    testOK ./blscosi_hybrid_rumor help
}
...
runCl(){
    ...
    dbgRun ./blscosi_hybrid_rumor -d $DBG_APP $@ -g $D
}

runSrvCfg(){
    echo -e "localhost:200$(( 2 * $1 ))\nCosi $1\n$(pwd)/srv$1\n" | ./blscosi_hybrid_rumor server setup > $OUT
}

runSrv(){
    ( ./blscosi_hybrid_rumor -d $DBG_SRV server -c srv$1/private.toml & )
