...
testServerCfg(){
    runSrvCfg 1
    pkill -9 blscosi_mask
    testFile srv1/private.toml
}

testBuild(){
    testOK ./blscosi_mask help
}
...
runCl(){
    ...
    dbgRun ./blscosi_mask -d $DBG_APP $@ -g $D
}

runSrvCfg(){
    echo -e "localhost:200$(( 2 * $1 ))\nCosi $1\n$(pwd)/srv$1\n" | ./blscosi_mask server setup > $OUT
}

runSrv(){
    ( ./blscosi_mask -d $DBG_SRV server -c srv$1/private.toml & )
