...
import (
	...
	"github.com/dedis/student_19_elias/blscosi_rumor"
...
func SignStatement(msg []byte, ro *onet.Roster) (*blscosi_rumor.SignatureResponse, error) {
	client := blscosi_rumor.NewClient()
	publics := ro.ServicePublics(blscosi_rumor.ServiceName)

	log.Lvlf4("Signing message %x", msg)

	pchan := make(chan *blscosi_rumor.SignatureResponse, 1)
...
func VerifySignatureHash(b []byte, sig *blscosi_rumor.SignatureResponse, ro *onet.Roster) error {
	suite := blscosi_rumor.NewClient().Suite().(*pairing.SuiteBn256)
	publics := ro.ServicePublics(blscosi_rumor.ServiceName)
