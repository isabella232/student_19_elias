...
testServerCfg(){
    runSrvCfg 1
    pkill -9 blscosi_maskaggr
    testFile srv1/private.toml
}

testBuild(){
    testOK ./blscosi_maskaggr help
}
...
runCl(){
    ...
    dbgRun ./blscosi_maskaggr -d $DBG_APP $@ -g $D
}

runSrvCfg(){
    echo -e "localhost:200$(( 2 * $1 ))\nCosi $1\n$(pwd)/srv$1\n" | ./blscosi_maskaggr server setup > $OUT
}

runSrv(){
    ( ./blscosi_maskaggr -d $DBG_SRV server -c srv$1/private.toml & )
